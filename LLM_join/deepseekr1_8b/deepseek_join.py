...
VECTOR_STORE_PATH = "C:\code\medical"  # index.faiss, index.pkl 있는 경로 설정
OLLAMA_MODEL_NAME = "deepseek-r1:8b"   # Ollama 에서 사용할 모델
...
try:
    ...
    llm.invoke("Test") # 연결 테스트
    logger.info(f"Ollama '{OLLAMA_MODEL_NAME}' LLM을 성공적으로 연결했습니다.")
except Exception as e:
    logger.error(f"Ollama LLM 연결 중 오류 발생: {e}")
    logger.error("🚨 Ollama가 실행 중인지 확인하세요.")
    exit()
